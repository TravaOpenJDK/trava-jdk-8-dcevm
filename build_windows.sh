...
cd ./openjdk-build
export LOG=info
export kate TOOLCHAIN_VERSION="2013"
./makejdk-any-platform.sh --tag "${SOURCE_JDK_TAG}" --build-variant dcevm  --branch dcevm11 --jdk-boot-dir "${JDK_BOOT_DIR}" --hswap-agent-download-url ${HSWAP_AGENT_DOWNLOAD_URL} --configure-args "--with-freetype-include=${FREETYPE_DIR}/include --with-freetype-lib=${FREETYPE_DIR}/win64 --disable-ccache --with-extra-cflags=/W0 --with-extra-cxxflags=/W0" --target-file-name java8-openjdk-dcevm-${TRAVIS_OS_NAME}.zip jdk8u
